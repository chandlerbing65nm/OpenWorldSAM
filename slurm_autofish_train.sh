...
#SBATCH --output=logs/a-autofish/output_%j.txt
...
OUTPUT_DIR=${OUTPUT_DIR:-./output_autofish_semseg}
RUN_IDX=${RUN_IDX:-0}
...
# ---------------------------------------------------------------------------
# Data loading / AutoFish dataset
# ---------------------------------------------------------------------------
NUM_WORKERS=${NUM_WORKERS:-4}
RANDOM_SUBSET_RATIO=${RANDOM_SUBSET_RATIO:-1.0}
IMAGE_SIZE=${IMAGE_SIZE:-1024}
AUTOFISH_DATASET_ROOT=${AUTOFISH_DATASET_ROOT:-/scratch/project_465002853/datasets/autofish/AutoFish}
AUTOFISH_MASKS_ROOT=${AUTOFISH_MASKS_ROOT:-/scratch/project_465002853/datasets/autofish/AutoFish/masks}
AUTOFISH_ANNOTATIONS_JSON=${AUTOFISH_ANNOTATIONS_JSON:-/scratch/project_465002853/datasets/autofish/AutoFish/annotations.json}
AUTOFISH_NUM_CLASSES=${AUTOFISH_NUM_CLASSES:-6}
...
SOLVER_STEPS=${SOLVER_STEPS:-"(50000, 70000)"}
...
# (slightly more permissive than COCO defaults to avoid filtering all masks)
# ---------------------------------------------------------------------------
...
export AUTOFISH_DATASET_ROOT
export AUTOFISH_MASKS_ROOT
export AUTOFISH_ANNOTATIONS_JSON
...
if [ "$RUN_MODE" = "train" ]; then
  python train_net.py \
    ...
    DATASETS.TRAIN "('autofish_sem_seg_train',)" \
    DATASETS.TEST "('autofish_sem_seg_val',)" \
    ...
    MODEL.SEM_SEG_HEAD.NUM_CLASSES "$AUTOFISH_NUM_CLASSES" \
